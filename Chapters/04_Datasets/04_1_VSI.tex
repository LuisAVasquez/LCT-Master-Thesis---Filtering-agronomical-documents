...
For each website, the tool \trafilatura{} \myparencite{barbaresi-2021-trafilatura} takes its XML and attempts to extract a title and  an abstract from the XML metadata\footnote{The title and the abstract are extracted from the \texttt{metadata.title} and \texttt{metadata.description} fields, respectively.} and the full text from the XML body.
